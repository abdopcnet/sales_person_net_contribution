	...
	try:
		...
		if sales_invoice_names:
			...
			if len(unique_invoices) > 1:
				frappe.throw(_("Multiple different Sales Invoices found. Please ensure only one invoice is referenced in all rows."))
		...
		if sales_order_names:
			...
			if len(unique_orders) > 1:
				frappe.throw(_("Multiple different Sales Orders found. Please ensure only one order is referenced in all rows."))
		...
		if not invoice_name:
			frappe.throw(_("No Sales Invoice or Sales Order found in references"))
		...
		if sales_invoice_name:
			...
			frappe.throw(_("Sales Order support will be added later. Please use Sales Invoice."))
		
		# Get total taxes and charges from Sales Invoice
		# Convert to float to handle string values from database
		try:
			total_taxes_and_charges = float(sales_invoice.total_taxes_and_charges or 0)
		except (ValueError, TypeError):
		...
		net_paid_after_all_deductions = total_paid - total_deductions - total_taxes_and_charges
		...
		if not hasattr(sales_invoice, 'sales_team') or not sales_invoice.sales_team:
			frappe.throw(_("No Sales Team found in Sales Invoice {0}").format(invoice_name))
		...
		grand_total_formatted = frappe.format_value(grand_total, {'fieldtype': 'Currency', 'currency': currency}, sales_invoice)
		total_taxes_formatted = frappe.format_value(total_taxes_and_charges, {'fieldtype': 'Currency', 'currency': currency}, sales_invoice)
		total_paid_formatted = frappe.format_value(total_paid, {'fieldtype': 'Currency', 'currency': currency}, payment_entry)
		total_deductions_formatted = frappe.format_value(total_deductions, {'fieldtype': 'Currency', 'currency': currency}, payment_entry)
		net_paid_formatted = frappe.format_value(net_paid, {'fieldtype': 'Currency', 'currency': currency}, payment_entry)
		net_paid_after_all_formatted = frappe.format_value(net_paid_after_all_deductions, {'fieldtype': 'Currency', 'currency': currency}, payment_entry)
		...
		for detail in sales_persons_details:
			...
			net_paid_formatted = flt(detail['net_paid_after_all_deductions'], precision=2)
		...
		frappe.log_error(frappe.get_traceback(), _("Error calculating net contribution"))
		frappe.throw(_("Error calculating net contribution: {0}").format(str(e)))
